...
def mine_block ():
    ...
    blockchain.add_transaction(sender=node_addess,receiver='Prairit',amount=1)
...
def is_valid():
    value = blockchain.is_valid_chain(blockchain.chain)
    if value is True:
...
#Decentralizing the cryptocurrency
...
def connect_node():
    ...
    if nodes is None:
        return "No nodes", 400
    for node in nodes:
        blockchain.add_block(node)
    response={'message':'All nodes have been connected.The list of the nodes are as follows:',
...
def replace_chain():
    ...
    if is_chain_replaced is True:
        ...
        response={'message':'All good.The chain is already the longest.',
